    ...
    for (let i = 0; i < targetLocations.length; i++) {
        if (distresult > 50) {
            if (place.visited == "true") {
                showDistanceToTarget.innerHTML = place.title + " [BESÖKT]"
            }
            else {
                showDistanceToTarget.innerHTML = "Du är " + distresult + " meter från " + place.title
            }
...
function drawPlace(id) {
    ...
    else if (foundPlace.id == 2) {
        points += 20
    }    
    else if (foundPlace.id == 3) {
        points += 25
    }
    else if (foundPlace.id == 4) {
        points += 30
    }
    else if (foundPlace.id >= 5) {
        points += 35
    }
...
/* function setVisited(id) {
    for (let i=0; i<targetLocations.length; i++) {
      if (targetLocations[i].Id == id) {
        targetLocations[i].visited = true;
        return targetLocations[i].visited;
      }
    }
  } */
...
function openStreetMap() {
    ...
    statusText.innerHTML = " Du är inom " + accuracy + " meter från denna position."
